...
class CustomDataset(data.Dataset):
    def __init__(self, root, phase='train'):
        ...
        self.label_path = os.path.join(root, self.phase, self.phase+'.csv') #data까지가 root
        with open(self.label_path, 'r', encoding='utf-8-sig') as f:
            ...
            for line in f.readlines()[1:]:
                ...
                if self.phase != 'test':
                    label_dict[v[0]] = (v[1],v[2]) # title_name, label
    ...
    def __getitem__(self, index):
        ...
        if self.phase != 'test':
            label = self.labels[self.imgs[index]][1]
            label = torch.tensor(int(label))

        transform = get_transform() #FIXME
